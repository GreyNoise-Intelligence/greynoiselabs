...
class Client(AsyncBaseClient):
    ...
    async def get_pivot(
        self,
        pcap: Upload,
        gnql: Union[Optional[bool], UnsetType] = UNSET,
        reverse: Union[Optional[bool], UnsetType] = UNSET,
    ) -> GetPivot:
        query = gql(
            """
            query getPivot($pcap: Upload!, $gnql: Boolean = false, $reverse: Boolean = false) {
              pivot(input: {file: $pcap, gnql: $gnql, reverse: $reverse}) {
                __typename
                ... on GNQLResult {
                  queries {
                    type
        ...
        variables: Dict[str, object] = {"pcap": pcap, "gnql": gnql, "reverse": reverse}
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
