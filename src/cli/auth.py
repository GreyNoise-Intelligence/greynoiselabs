...
def token_saver(token_filename, token_data):
    ...
    try:
        ...
        print(f"Error saving token to {token_filename}, {ex}")
...
def validate_token(id_token):
    ...
    tv = TokenVerifier(signature_verifier=sv, issuer=ISSUER_URL, audience=AUTH0_CLIENT_ID)
    try:
        tv.verify(id_token)
        return True
    except Exception as ex:
...
def login(token_filename):
    ...
    if token_data:
        ...
        if validated:
            ...
            # Attempt to refresh the token when possible, the one we have in the file is not valid.
        ...
        print("No token found, initiating device authentication.")
